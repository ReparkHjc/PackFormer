...
class SE_Decoder(nn.Module):
    def __init__(self,cell_num=3,embed_dim=1024):
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(12, 6, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(6, cell_num, kernel_size=1),
            nn.Sigmoid()
        )
        self.Flatten=nn.Flatten(1,3)
        self.Fc=nn.Linear(cell_num*embed_dim,96)
        # self.Fc1 = nn.Linear(int(cell_num * embed_dim*0.8), 100)
        self.Fc1=nn.Linear(96,1)

        self.norm=nn.BatchNorm2d(cell_num, eps=1e-05, momentum=0.1, affine=True)
...
class SE_VIT_Decoder(nn.Module):
    def __init__(self,type=1,cell_num=5,img_size=60,patch_size=2,in_c=3,embed_dim=500,
                     depth=3,num_heads=4,mlp_ratio=4,qkv_bias=False,attn_drop_ratio=0,
                     drop_path_ratio=0,norm_layer=None,act_layer=None):
        """
            Args:
                   type:1:SE_Decoder()
                        2:Flat_decoder()
        ...
        self.type=type
        if type==1:
            self.decoder=SE_Decoder(cell_num=cell_num,embed_dim=embed_dim)
