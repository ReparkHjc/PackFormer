...
# 区别在于train_num = idx_v[train_num]，是否选取随机数
# def train_val_dataset_sw2(SOH_path, data_path,Shuffle,Training_ratio):
#
#     data = pd.read_csv(data_path,header=None)
#     data.columns = ["t1", "q1", "T1", "t2", "q2", "T2", "t3", "q3",'T3']
#     SOH = pd.read_csv(SOH_path,header=None)
#     # Time,V,i,t
#     Train_num=int(Training_ratio*SOH.shape[0])
...
#
#     idx_v = []
#     idx_t = random.sample(range(0, 110), Train_num)
#     idx_t = py.sort(idx_t)
...
#
#         is_first_run =0
#         for train_num in range(Train_num,110):
#             t_1 = date_preprocess(t1[0][train_num * num:(sw_shape + train_num) * num], num * sw_shape, row_pitch, pitch_num)
#             q_1 = date_preprocess(q1[0][train_num * num:(sw_shape + train_num) * num], num * sw_shape, row_pitch, pitch_num)
#             T_1 = date_preprocess(T1[0][train_num * num:(sw_shape + train_num) * num], num * sw_shape, row_pitch, pitch_num)
#             data_tensor1 = con(t_1, q_1, T_1)
...
#
#         is_first_run = 0
#         for train_num in range(0,110-Train_num):
    ...
    if Shuffle == False:
        ...
        for train_num in range(Train_num,data_num):
            ...
            if is_first_run == 0:
                ...
                data_tensor = torch.cat([data_tensor,data_tensor_all], dim=0)
                a=SOH_list[train_num+20:train_num+20+prediction_num].reshape(1,-1)
                SOH=torch.cat([SOH,SOH_list[train_num+20:train_num+20+prediction_num].reshape(1,-1)])
