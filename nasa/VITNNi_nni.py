...
class SE_Decoder(nn.Module):
    ...
    def forward(self,x):
        ...
        #
        #     # 获取时间步和特征的数量
        #     num_time_steps = len(data_array[::9])
        #     time_steps = np.arange(num_time_steps)
        #     print(data_array)
        #     print("=" * 10)
        #     # 绘制柱状图，使用不同颜色
        #     colors = ['blue', 'green', 'orange', 'red', 'black']
        #     plt.bar(time_steps + i * 0.2, data_array[::9], color=colors[i], alpha=0.7, label=f'Subplot {i + 1}')
        #
        ...
        # plt.tight_layout()
        #
        # # save_dir2 = '../figure/plot_try/nasa'
        # # os.makedirs(save_dir2, exist_ok=True)
        # # plt.savefig(os.path.join(save_dir2, 'combined_bar_charts.png'))
        # plt.show()
        x=torch.relu(x)
        x = self.norm(x)
        x=self.Flatten(x)
...
class SE_VIT_Decoder(nn.Module):
    def __init__(self,type=1,cell_num=3,img_size=60,patch_size=6,in_c=3,embed_dim=100,
                     depth=4,num_heads=4,mlp_ratio=4,qkv_bias=False,attn_drop_ratio=0,
                     drop_path_ratio=0,norm_layer=None,act_layer=None,momentum=0.1,decoderFc_size=32,prediction_num=1):
    ...
    def forward(self,x):
        ...
        # [batchsize,sequence=3,1024]
        cell_all=torch.cat([cell1, cell2,cell3], dim=1)
