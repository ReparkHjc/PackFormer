...
def plotdata(Model="MLP"):
    ...
    if Model == "VIT":
        ...
        path1 = './Oxford_20_4/Model/VIT/shuffle_True/0.8_params/epoch_3300.params'

    if Model == "CNN_LSTM":
        # CNN_LSTM.RNN,CGU

        model = Net(cell_num=5, input_size=180, hidden_dim=25, num_layers=3, n_class=4).cuda()
        path1 = './Oxford_20_4/Model/CNN_LSTM/shuffle_True/0.8_params/epoch_1600.params'
    elif Model == "MLP":
        model = MLP(cell_num=5, n_feature=10800, n_hidden=33, n_class=4).cuda()

        path1 = './Oxford_20_4/Model/MLP/shuffle_True/0.8_params/epoch_1000.params'
    elif Model == "CNN":
        model = CNN(cell_num=5, n_class=4).cuda()
        path1 = './Oxford_20_4/Model/CNN/shuffle_True/0.8_params/epoch_1500.params'

    elif Model == "LSTM":
        model = LSTM(input_size=2700, n_class=4).cuda()
        path1 = './Oxford_20_4/Model/LSTM/shuffle_True/0.8_params/epoch_1500.params'

    model.load_state_dict(torch.load(path1))

    train_dataloader, val_dataloader = train_val_dataset_sw2(SOH_path, data_path, Shuffle=True,
                                                             Training_ratio=train_radio,
                                                             batch_size=6, row_pitch=2, sw_shape=20, prediction_num=4)
    ...
    print('Method is ' + Model + ', rmse is {}, mape is {}'.format(rmse, mape))
