...
# 数据路径
parser.add_argument('--data_dir', type=str, default="./data_mit/20/data.csv")
parser.add_argument('--SOH_dir', type=str, default="./data_mit/20/SOH.csv")


parser.add_argument('--model_shuffle_filename', type=str, default="./MIT_params_tuner/model/")
parser.add_argument('--loss_filename', type=str, default="./MIT_params_tuner/loss/")
...
if __name__ == "__main__":
    ...
    try:
        ...
        model = SE_VIT_Decoder(patch_size=params["patch_size"], embed_dim=params["embedding"], depth=params["depth"],
                               num_heads=para["num_heads"], attn_drop_ratio=params["attn_drop_ratio"],
                               momentum=para["momentum"],
                               decoderFc_size=params["decoderFc_size"], prediction_num=para["prediction_num"])

        train_main(learning_rate=para["lr"], SOH_path=para["SOH_dir"], data_path=para["data_dir"],
                   ...
                   batch_size=params["batch_size"], model=model,prediction_num=para["prediction_num"],sw_size=para["sw_size"],Shuffle=True)
